...
dump = ModuleDump('/Daten/101companies/101web/data/dumps/predicates_new.json')
dump.matches    = []
dump.failures   = []
dump.rules      = rules
dump.predicates = list(matchesCore.predicates)
...
def matchingFunc(sFile, tFile):
    ...
    if build(sFile, tFile):
        ...
        units = matchesCore.handleFile('predicates', dirname[len(const101.sRoot) + 1:], basename, rules)
...
#actual loop
loopOverDir(const101.sRoot, const101.tRoot, '.predicates.json', matchingFunc)
...
print str(matchesCore.noPredicateConstraints) + " predicate constraints checked."
print str(matchesCore.noPredicateConstraintsOk) + " predicate constraints succeeded."
