...
def text2Lists(text):
	...
	for textm in re.finditer("\[\[([^\[\|]+)(\|([^\[]+))?\]\]",text):
		...
		texts.append(text[textstart:textm.start()])
		links.append((linkm.group(2), linkm.group(3), textm.group(3)))
		textstart = textm.end()
	return dict(texts=texts, links=links)
		...
		for e in json[t]:
			for attr in json[t][e]:
				if attr in textsections:
					result = text2Lists(json[t][e][attr])
					json[t][e][attr] = result			
