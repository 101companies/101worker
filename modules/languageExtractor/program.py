...
output = os.path.join('../../../101web/data/resources', 'languages')
output = os.path.abspath(output)

def dump_to_databse(k, ref, old, new):
    with open('languages.json', 'w') as f:
        f.write(json.dumps(new, sort_keys=True, indent=4, separators=(',', ': '), cls=SetEncoder))

state = atom.Atom({})
state.add_watch('dump_to_databse', dump_to_databse)

def add_contribution(cur_state, lang, title):
    cur_state[lang]['contributions'].add(title)
    return cur_state

def add_lang(cur_state, lang):
    cur_state[lang] = {
        'contributions': set([])
    }
    return cur_state
...
def main(page):
    ...
    if page['namespace'] == 'Language':
        lang = page['page_title_namespace'].split(':')[1]
        state.swap(add_lang, lang)

    elif page['namespace'] == 'Contribution':
        title = page['page_title_namespace'].split(':')[1]
        for lang in state.deref().keys():
            if ('uses::Language:' + lang).lower() in page['raw_content'].lower():
                state.swap(add_contribution, lang, title)
