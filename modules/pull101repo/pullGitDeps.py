...
def copyContrib(dep):
    ...
    if dep.get('needsUpdate') or not os.path.exists(os.path.join(const101.sRoot, dep['targetdir'])):
        ...
        shutil.copytree(os.path.join(gitdepsFolder, name, dep['sourcedir'][1:]),
                        os.path.join(const101.sRoot, dep['targetdir']), ignore=ignoredFile)
...
def copySubdirs(dep):
    ...
    dirList = os.listdir(depsPath)
    for fname in dirList:
        if os.path.isdir(os.path.join(depsPath, fname)) and not '.git' in fname and not 'gradle' in fname:
...
for dep in gitdepsJson:
    ...
    status, output = commands.getstatusoutput(
        "./pullRepo.py " + os.path.join(gitdepsFolder, getRepoName(dep['sourcerepo'])) + " " + dep["sourcerepo"])
    if not output == '':
        print output
